            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_LAB_DEM WHERE DTB.Value.Tri
