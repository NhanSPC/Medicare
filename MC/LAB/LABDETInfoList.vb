...
Namespace MC
    ...
    <Serializable()>
    Public Class LABDETInfoList
        Inherits Csla.ReadOnlyListBase(Of LABDETInfoList, LABDETInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(LABDET).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As LABDETInfo In LABDETInfoList.GetLABDETInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, LABDET.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = LABDETInfoList.GetLABDETInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of LABDETInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As LABDETInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetLABDETInfo(ByVal pLineNo As String) As LABDETInfo
            Dim Info As LABDETInfo = LABDETInfo.EmptyLABDETInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetLABDETInfo(pLineNo).Description
        End Function

        Public Shared Function GetLABDETInfoList() As LABDETInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of LABDETInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetLABDETInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As LABDETInfo = Nothing) As Boolean

            RetInfo = LABDETInfo.EmptyLABDETInfo(pLineNo)
            If GetLABDETDic.ContainsKey(pLineNo) Then
                RetInfo = GetLABDETDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("LABDET"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_DETAIL_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_DETAIL_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_DETAIL_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = LABDETInfo.GetLABDETInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_LAB_DETAIL_DEM WHERE DTB.Value.Tri
...
#Region "LABDET Dictionary"

        Private Shared _LABDETDic As Dictionary(Of String, LABDETInfo)

        Private Shared Function GetLABDETDic() As Dictionary(Of String, LABDETInfo)
            If _LABDETDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _LABDETDic = New Dictionary(Of String, LABDETInfo)

                For Each itm In LABDETInfoList.GetLABDETInfoList
                    _LABDETDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_LAB_DETAIL_DEM  _DTB .Value.Tri
