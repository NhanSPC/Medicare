...
Namespace MC
    ...
    Public Class PRESCRIPTIONInfoList
        Inherits Csla.ReadOnlyListBase(Of PRESCRIPTIONInfoList, PRESCRIPTIONInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(PRESCRIPTION).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As PRESCRIPTIONInfo In PRESCRIPTIONInfoList.GetPRESCRIPTIONInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, PRESCRIPTION.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = PRESCRIPTIONInfoList.GetPRESCRIPTIONInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of PRESCRIPTIONInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As PRESCRIPTIONInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetPRESCRIPTIONInfo(ByVal pLineNo As String) As PRESCRIPTIONInfo
            Dim Info As PRESCRIPTIONInfo = PRESCRIPTIONInfo.EmptyPRESCRIPTIONInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetPRESCRIPTIONInfo(pLineNo).Description
        End Function

        Public Shared Function GetPRESCRIPTIONInfoList() As PRESCRIPTIONInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of PRESCRIPTIONInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetPRESCRIPTIONInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As PRESCRIPTIONInfo = Nothing) As Boolean

            RetInfo = PRESCRIPTIONInfo.EmptyPRESCRIPTIONInfo(pLineNo)
            If GetPRESCRIPTIONDic.ContainsKey(pLineNo) Then
                RetInfo = GetPRESCRIPTIONDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("PRESCRIPTION"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_PRESCRIPTION_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_PRESCRIPTION_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_PRESCRIPTION_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = PRESCRIPTIONInfo.GetPRESCRIPTIONInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_PRESCRIPTION_DEM WHERE DTB.Value.Tri
...
#Region "PRESCRIPTION Dictionary"

        Private Shared _prescriptionDic As Dictionary(Of String, PRESCRIPTIONInfo)

        Private Shared Function GetPRESCRIPTIONDic() As Dictionary(Of String, PRESCRIPTIONInfo)
            If _prescriptionDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _prescriptionDic = New Dictionary(Of String, PRESCRIPTIONInfo)

                For Each itm In PRESCRIPTIONInfoList.GetPRESCRIPTIONInfoList
                    _prescriptionDic.Add(itm.Code, itm)
                Next
            End If

            Return _prescriptionDic
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_PRESCRIPTION_DEM  _DTB .Value.Tri
