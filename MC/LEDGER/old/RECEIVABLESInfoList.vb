...
Namespace MC
    ...
    <Serializable()> _
    Public Class RECEIVABLESInfoList
        Inherits Csla.ReadOnlyListBase(Of RECEIVABLESInfoList, RECEIVABLESInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(RECEIVABLES).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As RECEIVABLESInfo In RECEIVABLESInfoList.GetRECEIVABLESInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, RECEIVABLES.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = RECEIVABLESInfoList.GetRECEIVABLESInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of RECEIVABLESInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As RECEIVABLESInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetRECEIVABLESInfo(ByVal pLineNo As String) As RECEIVABLESInfo
            Dim Info As RECEIVABLESInfo = RECEIVABLESInfo.EmptyRECEIVABLESInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetRECEIVABLESInfo(pLineNo).Description
        End Function

        Public Shared Function GetRECEIVABLESInfoList() As RECEIVABLESInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of RECEIVABLESInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetRECEIVABLESInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As RECEIVABLESInfo = Nothing) As Boolean

            RetInfo = RECEIVABLESInfo.EmptyRECEIVABLESInfo(pLineNo)
            If GetRECEIVABLESDic.ContainsKey(pLineNo) Then
                RetInfo = GetRECEIVABLESDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("RECEIVABLES"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_RECEIVABLES_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_MC_RECEIVABLES_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_RECEIVABLES_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = RECEIVABLESInfo.GetRECEIVABLESInfo(dr)
                                Me.Add(Info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_RECEIVABLES_DEM WHERE DTB.Value.Tri
...
#Region "RECEIVABLES Dictionary"

        Private Shared _receivablesDic As Dictionary(Of String, RECEIVABLESInfo)

        Private Shared Function GetRECEIVABLESDic() As Dictionary(Of String, RECEIVABLESInfo)
            If _receivablesDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _receivablesDic = New Dictionary(Of String, RECEIVABLESInfo)

                For Each itm In RECEIVABLESInfoList.GetRECEIVABLESInfoList
                    _receivablesDic.Add(itm.Code, itm)
                Next
            End If

            Return _receivablesDic
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_RECEIVABLES_DEM  _DTB .Value.Tri
