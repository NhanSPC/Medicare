...
Namespace MC
    ...
    <Serializable()> _
    Public Class SAMPLEPRESCInfoList
        Inherits Csla.ReadOnlyListBase(Of SAMPLEPRESCInfoList, SAMPLEPRESCInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(SAMPLEPRESC).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As SAMPLEPRESCInfo In SAMPLEPRESCInfoList.GetSAMPLEPRESCInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, SAMPLEPRESC.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = SAMPLEPRESCInfoList.GetSAMPLEPRESCInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of SAMPLEPRESCInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As SAMPLEPRESCInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetSAMPLEPRESCInfo(ByVal pLineNo As String) As SAMPLEPRESCInfo
            Dim Info As SAMPLEPRESCInfo = SAMPLEPRESCInfo.EmptySAMPLEPRESCInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetSAMPLEPRESCInfo(pLineNo).Description
        End Function

        Public Shared Function GetSAMPLEPRESCInfoList() As SAMPLEPRESCInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of SAMPLEPRESCInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetSAMPLEPRESCInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As SAMPLEPRESCInfo = Nothing) As Boolean

            RetInfo = SAMPLEPRESCInfo.EmptySAMPLEPRESCInfo(pLineNo)
            If GetSAMPLEPRESCDic.ContainsKey(pLineNo) Then
                RetInfo = GetSAMPLEPRESCDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("SAMPLEPRESC"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_SAMPLE_PRESCRIPTION_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_MC_SAMPLE_PRESCRIPTION_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_SAMPLE_PRESCRIPTION_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = SAMPLEPRESCInfo.GetSAMPLEPRESCInfo(dr)
                                Me.Add(Info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_SAMPLE_PRESCRIPTION_DEM WHERE DTB.Value.Tri
...
#Region "SAMPLEPRESC Dictionary"

        Private Shared _sampleprescDic As Dictionary(Of String, SAMPLEPRESCInfo)

        Private Shared Function GetSAMPLEPRESCDic() As Dictionary(Of String, SAMPLEPRESCInfo)
            If _sampleprescDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _sampleprescDic = New Dictionary(Of String, SAMPLEPRESCInfo)

                For Each itm In SAMPLEPRESCInfoList.GetSAMPLEPRESCInfoList
                    _sampleprescDic.Add(itm.Code, itm)
                Next
            End If

            Return _sampleprescDic
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_SAMPLE_PRESCRIPTION_DEM  _DTB .Value.Tri
