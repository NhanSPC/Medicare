...
Namespace MC
    ...
    <Serializable()> _
    Public Class FEEMANAGEMENTInfoList
        Inherits Csla.ReadOnlyListBase(Of FEEMANAGEMENTInfoList, FEEMANAGEMENTInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(FEEMANAGEMENT).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As FEEMANAGEMENTInfo In FEEMANAGEMENTInfoList.GetFEEMANAGEMENTInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, FEEMANAGEMENT.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = FEEMANAGEMENTInfoList.GetFEEMANAGEMENTInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of FEEMANAGEMENTInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As FEEMANAGEMENTInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetFEEMANAGEMENTInfo(ByVal pLineNo As String) As FEEMANAGEMENTInfo
            Dim Info As FEEMANAGEMENTInfo = FEEMANAGEMENTInfo.EmptyFEEMANAGEMENTInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetFEEMANAGEMENTInfo(pLineNo).Description
        End Function

        Public Shared Function GetFEEMANAGEMENTInfoList() As FEEMANAGEMENTInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of FEEMANAGEMENTInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetFEEMANAGEMENTInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As FEEMANAGEMENTInfo = Nothing) As Boolean

            RetInfo = FEEMANAGEMENTInfo.EmptyFEEMANAGEMENTInfo(pLineNo)
            If GetFEEMANAGEMENTDic.ContainsKey(pLineNo) Then
                RetInfo = GetFEEMANAGEMENTDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("FEEMANAGEMENT"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_FEE_MANAGEMENT_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_MC_FEE_MANAGEMENT_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_FEE_MANAGEMENT_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = FEEMANAGEMENTInfo.GetFEEMANAGEMENTInfo(dr)
                                Me.Add(Info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_FEE_MANAGEMENT_DEM WHERE DTB.Value.Tri
...
#Region "FEEMANAGEMENT Dictionary"

        Private Shared _feemanagementDic As Dictionary(Of String, FEEMANAGEMENTInfo)

        Private Shared Function GetFEEMANAGEMENTDic() As Dictionary(Of String, FEEMANAGEMENTInfo)
            If _feemanagementDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _feemanagementDic = New Dictionary(Of String, FEEMANAGEMENTInfo)

                For Each itm In FEEMANAGEMENTInfoList.GetFEEMANAGEMENTInfoList
                    _feemanagementDic.Add(itm.Code, itm)
                Next
            End If

            Return _feemanagementDic
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_FEE_MANAGEMENT_DEM  _DTB .Value.Tri
