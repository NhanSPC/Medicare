...
Namespace MC
    ...
    Public Class TRANSFERInfoList
        Inherits Csla.ReadOnlyListBase(Of TRANSFERInfoList, TRANSFERInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(TRANSFER).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As TRANSFERInfo In TRANSFERInfoList.GetTRANSFERInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, TRANSFER.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = TRANSFERInfoList.GetTRANSFERInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of TRANSFERInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As TRANSFERInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetTRANSFERInfo(ByVal pLineNo As String) As TRANSFERInfo
            Dim Info As TRANSFERInfo = TRANSFERInfo.EmptyTRANSFERInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetTRANSFERInfo(pLineNo).Description
        End Function

        Public Shared Function GetTRANSFERInfoList() As TRANSFERInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of TRANSFERInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetTRANSFERInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As TRANSFERInfo = Nothing) As Boolean

            RetInfo = TRANSFERInfo.EmptyTRANSFERInfo(pLineNo)
            If GetTRANSFERDic.ContainsKey(pLineNo) Then
                RetInfo = GetTRANSFERDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("TRANSFER"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_TRANSFER_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_TRANSFER_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_TRANSFER_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = TRANSFERInfo.GetTRANSFERInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_TRANSFER_DEM WHERE DTB.Value.Tri
...
#Region "TRANSFER Dictionary"

        Private Shared _transferDic As Dictionary(Of String, TRANSFERInfo)

        Private Shared Function GetTRANSFERDic() As Dictionary(Of String, TRANSFERInfo)
            If _transferDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _transferDic = New Dictionary(Of String, TRANSFERInfo)

                For Each itm In TRANSFERInfoList.GetTRANSFERInfoList
                    _transferDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_TRANSFER_DEM  _DTB .Value.Tri
