...
Namespace HR
    ...
    <Serializable()> _
    Public Class OFFTYPEInfoList
        Inherits Csla.ReadOnlyListBase(Of OFFTYPEInfoList, OFFTYPEInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(OFFTYPE).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As OFFTYPEInfo In OFFTYPEInfoList.GetOFFTYPEInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, OFFTYPE.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = OFFTYPEInfoList.GetOFFTYPEInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of OFFTYPEInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As OFFTYPEInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetOFFTYPEInfo(ByVal pLineNo As String) As OFFTYPEInfo
            Dim Info As OFFTYPEInfo = OFFTYPEInfo.EmptyOFFTYPEInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetOFFTYPEInfo(pLineNo).Description
        End Function

        Public Shared Function GetOFFTYPEInfoList() As OFFTYPEInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of OFFTYPEInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetOFFTYPEInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As OFFTYPEInfo = Nothing) As Boolean

            RetInfo = OFFTYPEInfo.EmptyOFFTYPEInfo(pLineNo)
            If GetOFFTYPEDic.ContainsKey(pLineNo) Then
                RetInfo = GetOFFTYPEDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("OFFTYPE"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_HR_OFF_TYPE_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_HR_OFF_TYPE_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_HR_OFF_TYPE_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = OFFTYPEInfo.GetOFFTYPEInfo(dr)
                                Me.Add(Info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_HR_OFF_TYPE_DEM WHERE DTB.Value.Tri
...
#Region "OFFTYPE Dictionary"

        Private Shared _offtypeDic As Dictionary(Of String, OFFTYPEInfo)

        Private Shared Function GetOFFTYPEDic() As Dictionary(Of String, OFFTYPEInfo)
            If _offtypeDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _offtypeDic = New Dictionary(Of String, OFFTYPEInfo)

                For Each itm In OFFTYPEInfoList.GetOFFTYPEInfoList
                    _offtypeDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_HR_OFF_TYPE_DEM  _DTB .Value.Tri
