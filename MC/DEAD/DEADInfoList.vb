...
Namespace MC
    ...
    <Serializable()>
    Public Class DEADInfoList
        Inherits Csla.ReadOnlyListBase(Of DEADInfoList, DEADInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(DEAD).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As DEADInfo In DEADInfoList.GetDEADInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, DEAD.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = DEADInfoList.GetDEADInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of DEADInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As DEADInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetDEADInfo(ByVal pLineNo As String) As DEADInfo
            Dim Info As DEADInfo = DEADInfo.EmptyDEADInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetDEADInfo(pLineNo).Description
        End Function

        Public Shared Function GetDEADInfoList() As DEADInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of DEADInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetDEADInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As DEADInfo = Nothing) As Boolean

            RetInfo = DEADInfo.EmptyDEADInfo(pLineNo)
            If GetDEADDic.ContainsKey(pLineNo) Then
                RetInfo = GetDEADDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("DEAD"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_DEAD_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_DEAD_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_DEAD_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = DEADInfo.GetDEADInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_DEAD_DEM WHERE DTB.Value.Tri
...
#Region "DEAD Dictionary"

        Private Shared _deadDic As Dictionary(Of String, DEADInfo)

        Private Shared Function GetDEADDic() As Dictionary(Of String, DEADInfo)
            If _deadDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _deadDic = New Dictionary(Of String, DEADInfo)

                For Each itm In DEADInfoList.GetDEADInfoList
                    _deadDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_DEAD_DEM  _DTB .Value.Tri
