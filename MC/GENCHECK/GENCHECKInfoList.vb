...
Namespace MC
    ...
    <Serializable()> _
    Public Class GENCHECKInfoList
        Inherits Csla.ReadOnlyListBase(Of GENCHECKInfoList, GENCHECKInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(GENCHECK).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As GENCHECKInfo In GENCHECKInfoList.GetGENCHECKInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, GENCHECK.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = GENCHECKInfoList.GetGENCHECKInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of GENCHECKInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As GENCHECKInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetGENCHECKInfo(ByVal pLineNo As String) As GENCHECKInfo
            Dim Info As GENCHECKInfo = GENCHECKInfo.EmptyGENCHECKInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetGENCHECKInfo(pLineNo).Description
        End Function

        Public Shared Function GetGENCHECKInfoList() As GENCHECKInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of GENCHECKInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetGENCHECKInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As GENCHECKInfo = Nothing) As Boolean

            RetInfo = GENCHECKInfo.EmptyGENCHECKInfo(pLineNo)
            If GetGENCHECKDic.ContainsKey(pLineNo) Then
                RetInfo = GetGENCHECKDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("GENCHECK"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_GENCHECK_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_MC_GENERAL_CHECK_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_GENERAL_CHECK_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = GENCHECKInfo.GetGENCHECKInfo(dr)
                                Me.Add(Info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_GENERAL_CHECK_DEM WHERE DTB.Value.Tri
...
#Region "GENCHECK Dictionary"

        Private Shared _GENCHECKDic As Dictionary(Of String, GENCHECKInfo)

        Private Shared Function GetGENCHECKDic() As Dictionary(Of String, GENCHECKInfo)
            If _GENCHECKDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _GENCHECKDic = New Dictionary(Of String, GENCHECKInfo)

                For Each itm In GENCHECKInfoList.GetGENCHECKInfoList
                    _GENCHECKDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_GENERAL_CHECK_DEM  _DTB .Value.Tri
