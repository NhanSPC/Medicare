...
Namespace MC
    ...
    <Serializable()>
    Public Class LABTESTInfoList
        Inherits Csla.ReadOnlyListBase(Of LABTESTInfoList, LABTESTInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(LABTEST).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As LABTESTInfo In LABTESTInfoList.GetLABTESTInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, LABTEST.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = LABTESTInfoList.GetLABTESTInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of LABTESTInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As LABTESTInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetLABTESTInfo(ByVal pLineNo As String) As LABTESTInfo
            Dim Info As LABTESTInfo = LABTESTInfo.EmptyLABTESTInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetLABTESTInfo(pLineNo).Description
        End Function

        Public Shared Function GetLABTESTInfoList() As LABTESTInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of LABTESTInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetLABTESTInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As LABTESTInfo = Nothing) As Boolean

            RetInfo = LABTESTInfo.EmptyLABTESTInfo(pLineNo)
            If GetLABTESTDic.ContainsKey(pLineNo) Then
                RetInfo = GetLABTESTDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("LABTEST"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(Database.PhoebusConnection)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_TEST_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_TEST_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim And TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        'cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_MC_LAB_TEST_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = LABTESTInfo.GetLABTESTInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    '    cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_MC_LAB_TEST_DEM WHERE DTB.Value.Tri
...
#Region "LABTEST Dictionary"

        Private Shared _labtestDic As Dictionary(Of String, LABTESTInfo)

        Private Shared Function GetLABTESTDic() As Dictionary(Of String, LABTESTInfo)
            If _labtestDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _labtestDic = New Dictionary(Of String, LABTESTInfo)

                For Each itm In LABTESTInfoList.GetLABTESTInfoList
                    _labtestDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_MC_LAB_TEST_DEM  _DTB .Value.Tri
